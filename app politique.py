...
def calcul():
    ...
    temp1=aleatoire1(ix1,iy1,iz1)
    fm=f1(temp1)
    Accm=Acc*fm
    listeAcm.append(int(Accm))
    temp2=aleatoire2(ix2,iy2,iz2)
    ft=f2(temp2)
    nbre_tue=Acc*ft
    listetue.append(int(nbre_tue))
    temp3=aleatoire3(ix3, iy3, iz3)
    nbre_blesse=Acc*f3(temp3)
    listeblesse.append(int(nbre_blesse))
    temp4=aleatoire4(ix4, iy4, iz4)
    nbre_grave=nbre_blesse*f4(temp4)
    listebg.append(int(nbre_grave))
    nbre_leger=nbre_blesse-nbre_grave  
    listebl.append(int(nbre_leger))
    print("leger",nbre_leger)
    H=H+1
    while (H < dep):
        print(H)
        temp=aleatoire(ix,iy,iz)
        Acc+=Acc*f(temp)
        temp1=aleatoire1(ix1,iy1,iz1)
        fm=f1(temp1)
        Accm=Acc*fm
...
def estimer():
    ...
    while i<=m:
        ...
        fm=f1(aleatoire1(ix1,iy1,iz1))
        Accm=Acc*fm
        ft=f2(aleatoire2(ix2,iy2,iz2))
        nbre_tue=Acc*ft
        nbre_blesse=Acc*f3(aleatoire3(ix3, iy3, iz3))
        nbre_grave=nbre_blesse*f4(aleatoire4(ix4, iy4, iz4))
        nbre_leger=nbre_blesse-nbre_grave      
        H=H+1   
        while H<=end:
            Acc+=Acc*f(aleatoire(ix,iy,iz))
            fm=f1(aleatoire1(ix1,iy1,iz1))
            Accm=Acc*fm
            ft=f2(aleatoire2(ix1,iy1,iz1))
            nbre_tue=Acc*ft
            nbre_blesse=Acc*f3(aleatoire3(ix3, iy3, iz3))
            nbre_grave=nbre_blesse*f4(aleatoire4(ix4, iy4, iz4))
            nbre_leger=nbre_blesse-nbre_grave            
